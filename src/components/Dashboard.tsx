...
import { Trophy, TrendingUp, DollarSign, Calendar, ArrowUpRight, ArrowDownRight, MoreHorizontal, Filter, Target, User } from 'lucide-react';
...
export const Dashboard = () => {
  ...
  const rankingData = useMemo(() => {
    return users.map(user => {
      ...
      // Determine Team Goal (Approximate by dividing team goal by members, or just generic logic)
  ...
  const productData = useMemo(() => {
    ...
    wonLeads.forEach(lead => {
      // Assuming the first tag is the "Product"
  ...
  // 7. Evolution Data (Mocked time distribution based on created date)
                       ...
                       {rankingData.map((user, idx) => (
                           <tr key={user.id} className="hover:bg-blue-50/30 transition-colors">
                               ...
                               <td className="px-4 py-3 text-right font-bold text-gray-800">
                                   {currencyFormatter(user.revenue)}
        ...
        <div className="lg:col-span-3 bg-gray-900 rounded-xl shadow-lg border border-gray-800 text-white flex flex-col relative overflow-hidden">
            ...
            <div className="p-6 bg-gray-800/50 mt-auto border-t border-gray-800 z-10">
                 <div className="flex justify-between text-xs mb-2">
                     <span className="text-green-400 font-bold">{currencyFormatter(totalRevenue)}</span>
                     <span className="text-gray-500">Meta: {currencyFormatter(globalGoal)}</span>
                 </div>
                 ...
                 <div className="text-center mt-3">
                     <span className="text-xs bg-gray-700 text-gray-300 px-2 py-1 rounded">
                         Gap: {gap > 0 ? `-${currencyFormatter(gap)}` : 'Meta Batida! 🚀'}
                     </span>
                 </div>
            </div>
        </div>
                           ...
                           return (
                               <tr key={lead.id} className="hover:bg-gray-50">
                                   ...
                                   <td className="px-4 py-3 text-gray-600 flex items-center gap-2">
                                       <User size={12} /> {rep?.name.split(' ')[0] || 'N/A'}
                                   </td>
                                   <td className="px-4 py-3 text-right font-bold text-green-600">
                                       {currencyFormatter(lead.value)}
                                   </td>
                               </tr>
                           );
                       })}
        ...
        <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col">
            ...
            <div className="mt-4 pt-4 border-t border-gray-100 flex gap-6 text-sm">
                <div>
                   ...
                   <span className="font-bold text-gray-800">{currencyFormatter(activeLeads.reduce((a,b)=>a+b.value,0))}</span>
                </div>
                <div>
                   <span className="text-gray-500 block text-xs">Total Ganho</span>
                   <span className="font-bold text-green-600">{currencyFormatter(totalRevenue)}</span>
                </div>
            </div>
        </div>
            ...
            <div className="flex-1 min-h-[200px] relative">
                <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                        ...
                        <Tooltip formatter={(val: number) => currencyFormatter(val)} />
              ...
              <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={evolutionData}>
                      ...
                      <Tooltip 
                        ...
                        formatter={(val: number) => [currencyFormatter(val), 'Vendas']}
                      />
