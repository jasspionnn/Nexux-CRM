...
interface CRMContextType {
  ...
  // Fix: Missing properties required by Settings and Teams components
...
export const CRMProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const refreshData = useCallback(async () => {
      ...
      try {
          if (currentUser.role === UserRole.NEXUS_ADMIN) {
               ...
               console.log(`[FRONTEND] Buscando dados para conta: ${currentUser.accountId}`);
  ...
  // Fix: Implement missing properties required by Settings and Teams components
  const updateVisibilitySettings = async (updates: any) => {
    if (!currentUser?.accountId) return;
    await api.patch(`/accounts/${currentUser.accountId}/visibility`, updates);
    refreshData();
  };
  ...
  const reorderStages = async (funnelId: string, stageIds: string[]) => {
    const funnel = funnels.find(f => f.id === funnelId);
    if (!funnel) return;
  ...
  return (
    <CRMContext.Provider value={{
      ...
      // Fix: Exporting missing properties
      updateVisibilitySettings, addUser, addTeam, updateTeam, deleteTeam, deleteUser, reorderStages
    }}>
      {children}
    </CRMContext.Provider>
  );
