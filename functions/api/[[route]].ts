...
app.post('/auth/register', async (c) => {
    ...
    await c.env.DB.prepare('INSERT INTO accounts (id, company_name, owner_name, email) VALUES (?, ?, ?, ?)')
        .bind(accountId, companyName, userName, email).run();
    
    await c.env.DB.prepare('INSERT INTO users (id, account_id, name, email, password, role, avatar) VALUES (?, ?, ?, ?, ?, ?, ?)')
        .bind(userId, accountId, userName, email, password, 'ACCOUNT_ADMIN', `https://ui-avatars.com/api/?name=${userName}`).run();
...
app.get('/sync/:accountId', async (c) => {
    ...
    return c.json({
        ...
        customFields: customFields.results.map((cf: any) => ({
            id: cf.id, name: cf.name, type: cf.type, context: cf.context, funnelId: cf.funnel_id,
            options: JSON.parse(cf.options || '[]'), visibleStageIds: JSON.parse(cf.visible_stage_ids || '[]')
        }))
    });
});
