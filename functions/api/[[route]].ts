...
// Auxiliares de parsing para o SQLite (D1)
...
app.get('/sync/:accountId', async (c) => {
    ...
    try {
        const [funnelsRes, stagesRes, leadsRes, fieldsRes] = await Promise.all([
        ...
        return c.json({ funnels, leads, customFields: fieldsRes.results });
    } catch (e: any) {
...
app.patch('/leads/:id', async (c) => {
    ...
    // Mapeamento dinâmico simples para campos snake_case
    const mapping: any = {
