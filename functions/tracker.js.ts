  ...
  const script = `(function(w){
    var T={
      ...
      _send:function(type,data){
        ...
        if(type==='form')console.log('[NX] Form captured:',data.form_data?data.form_data.fid:'unknown');
      ...
      _captureForm:function(f,convKeys){
        ...
        f._nxTracked=true;
        var fd={},hasConv=false,convFields={},fid=f.id||'form_'+T._hash(f.action||window.location.pathname);
        ...
        T._send('form',{url:w.location.href,ref:document.referrer||null,form_data:{fid:fid,action:action,fields:fd,has_lead:hasConv}});
        if(hasConv){
