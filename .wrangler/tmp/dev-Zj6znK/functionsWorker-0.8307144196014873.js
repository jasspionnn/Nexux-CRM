...
// .wrangler/tmp/bundle-pGnyKW/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/pages-RCuZvk/functionsWorker-0.8307144196014873.mjs
...
app.get("/debug-schema", async (c) => {
  try {
    ...
    if (table === "stages")
      query = "PRAGMA table_info(stages)";
    if (table === "funnels")
      query = "PRAGMA table_info(funnels)";
    if (table === "leads")
...
app.get("/seed-db", async (c) => {
  try {
    await c.env.DB.prepare(`
      ...
      VALUES ('acc_demo', 'Tech Solutions Ltda', 'Jo\xE3o Silva', 'joao@tech.com', 'active', 'pro', '2025-12-31T23:59:59Z', datetime('now'))
      ON CONFLICT(id) DO NOTHING;
    `).run();
    const existingUser = await c.env.DB.prepare("SELECT id FROM users WHERE email = ?").bind("joao@tech.com").first();
    if (!existingUser) {
      await c.env.DB.prepare(`
        INSERT INTO users (id, account_id, name, email, password, role, avatar, status, joined_at)
        VALUES ('u_owner', 'acc_demo', 'Jo\xE3o Silva', 'joao@tech.com', '123456', 'ACCOUNT_ADMIN', 'https://ui-avatars.com/api/?name=Joao+Silva&background=0D8ABC&color=fff', 'active', '2023-01-01T10:00:00Z')
        ON CONFLICT(id) DO NOTHING;
      `).run();
    }
    await c.env.DB.prepare(`
      INSERT INTO funnels (id, account_id, name) VALUES ('f_vendas', 'acc_demo', 'Funil de Vendas Padr\xE3o')
      ON CONFLICT(id) DO NOTHING;
    `).run();
    await c.env.DB.prepare(`
  ...
  try {
    ...
    await c.env.DB.prepare(`
      CREATE TABLE IF NOT EXISTS bot_settings (
          ...
          system_prompt TEXT DEFAULT 'Voc\xEA \xE9 um assistente de vendas gentil. Use as informa\xE7\xF5es fornecidas para tirar d\xFAvidas.',
    ...
    try {
      await c.env.DB.prepare("ALTER TABLE accounts ADD COLUMN expires_at TEXT").run();
    } catch (e) {
    }
    try {
      await c.env.DB.prepare(`ALTER TABLE accounts ADD COLUMN created_at TEXT DEFAULT (datetime('now'))`).run();
    } catch (e) {
    }
    try {
    ...
    try {
      await c.env.DB.prepare("ALTER TABLE stages ADD COLUMN color TEXT").run();
    } catch (e) {
    }
    try {
      await c.env.DB.prepare("ALTER TABLE leads ADD COLUMN custom_values TEXT").run();
    } catch (e) {
    }
    try {
      await c.env.DB.prepare(`ALTER TABLE leads ADD COLUMN created_at TEXT DEFAULT (datetime('now'))`).run();
    } catch (e) {
    }
    try {
...
app.get("/funnels", async (c) => {
  ...
  const funnelsWithStages = results.map((funnel) => ({
    ...funnel,
    stages: stages.filter((stage) => stage.funnel_id === funnel.id)
  }));
...
app.post("/funnels", async (c) => {
  ...
  await c.env.DB.prepare("INSERT INTO funnels (id, account_id, name) VALUES (?, ?, ?)").bind(id, account_id, body.name).run();
  return c.json({ id, name: body.name, stages: [] });
});
app.put("/funnels/:id", async (c) => {
  const id = c.req.param("id");
  const body = await c.req.json();
  await c.env.DB.prepare("UPDATE funnels SET name = ? WHERE id = ?").bind(body.name, id).run();
  return c.json({ success: true });
});
...
app.post("/funnels/:funnelId/stages", async (c) => {
  ...
  const body = await c.req.json();
  const id = crypto.randomUUID();
  await c.env.DB.prepare('INSERT INTO stages (id, funnel_id, name, color, "order") VALUES (?, ?, ?, ?, ?)').bind(id, funnelId, body.name, body.color, body.order || 0).run();
  return c.json({ id, funnel_id: funnelId, name: body.name, color: body.color, order: body.order || 0 });
});
app.put("/stages/:id", async (c) => {
  const id = c.req.param("id");
  const body = await c.req.json();
  await c.env.DB.prepare("UPDATE stages SET name = ?, color = ? WHERE id = ?").bind(body.name, body.color, id).run();
  return c.json({ success: true });
});
...
app.get("/webhooks", async (c) => {
  ...
  return c.json(results.map((w) => ({ ...w, active: w.is_active === 1 })));
});
app.post("/webhooks", async (c) => {
  try {
    ...
    if (!stage_id) {
      const stage = await c.env.DB.prepare("SELECT id FROM stages WHERE funnel_id = ? LIMIT 1").bind(funnel_id).first();
      stage_id = stage ? stage.id : "s_contato";
    }
    await c.env.DB.prepare("INSERT INTO webhooks (id, account_id, name, url, events, is_active, funnel_id, stage_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)").bind(id, account_id, body.name, body.url, "all", body.active ? 1 : 0, funnel_id, stage_id).run();
...
app.put("/webhooks/:id", async (c) => {
  const id = c.req.param("id");
  const body = await c.req.json();
  await c.env.DB.prepare("UPDATE webhooks SET name = ?, url = ?, is_active = ? WHERE id = ?").bind(body.name, body.url, body.active ? 1 : 0, id).run();
  return c.json({ success: true });
});
...
app.post("/users", async (c) => {
  try {
    ...
    await c.env.DB.prepare("INSERT INTO users (id, account_id, name, email, password, role, status, team_id, avatar, joined_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))").bind(id, account_id, body.name, body.email, "temp_password", body.role, body.status, team_id, avatar).run();
...
app.put("/users/:id", async (c) => {
  const id = c.req.param("id");
  const body = await c.req.json();
  await c.env.DB.prepare("UPDATE users SET name = ?, email = ?, role = ?, status = ? WHERE id = ?").bind(body.name, body.email, body.role, body.status, id).run();
  return c.json({ success: true });
});
...
app.post("/leads", async (c) => {
  try {
    ...
    `).bind(
      id,
      "acc_demo",
      // Using the same demo account ID
      body.funnel_id,
      body.stage_id,
      body.title || "Nova Negocia\xE7\xE3o",
      body.company || "",
      body.value || 0,
...
app.post("/leads/:id/notes", async (c) => {
  ...
  const body = await c.req.json();
  const id = crypto.randomUUID();
  await c.env.DB.prepare("INSERT INTO notes (id, lead_id, content, author_name, created_at) VALUES (?, ?, ?, ?, datetime('now'))").bind(id, leadId, body.content, body.author_name || "Usu\xE1rio").run();
...
app.post("/tasks", async (c) => {
  const body = await c.req.json();
  const id = crypto.randomUUID();
  await c.env.DB.prepare("INSERT INTO tasks (id, lead_id, title, due_date, completed, type) VALUES (?, ?, ?, ?, ?, ?)").bind(id, body.lead_id, body.title, body.due_date, body.completed ? 1 : 0, body.type || "task").run();
...
app.post("/teams", async (c) => {
  ...
  const id = crypto.randomUUID();
  const account_id = "acc_demo";
  await c.env.DB.prepare("INSERT INTO teams (id, account_id, name, goal) VALUES (?, ?, ?, ?)").bind(id, account_id, body.name, body.goal || 0).run();
  return c.json({ id, account_id, name: body.name, goal: body.goal || 0 });
});
app.put("/teams/:id", async (c) => {
  const id = c.req.param("id");
  const body = await c.req.json();
  await c.env.DB.prepare("UPDATE teams SET name = ?, goal = ? WHERE id = ?").bind(body.name, body.goal, id).run();
  return c.json({ success: true });
});
...
app.put("/bot-settings", async (c) => {
  ...
  if (existing) {
    await c.env.DB.prepare("UPDATE bot_settings SET system_prompt = ?, temperature = ?, auto_reply = ?, whatsapp_webhook_token = ? WHERE account_id = ?").bind(body.system_prompt, body.temperature, body.auto_reply ? 1 : 0, body.whatsapp_webhook_token, account_id).run();
  } else {
    await c.env.DB.prepare("INSERT INTO bot_settings (account_id, system_prompt, temperature, auto_reply, whatsapp_webhook_token) VALUES (?, ?, ?, ?, ?)").bind(account_id, body.system_prompt, body.temperature, body.auto_reply ? 1 : 0, body.whatsapp_webhook_token).run();
  }
  return c.json({ success: true });
});
...
app.post("/knowledge-sources", async (c) => {
  ...
  const id = crypto.randomUUID();
  const account_id = "acc_demo";
  await c.env.DB.prepare("INSERT INTO knowledge_sources (id, account_id, name, type) VALUES (?, ?, ?, ?)").bind(id, account_id, body.name, body.type).run();
...
app.post("/knowledge-sources/:sourceId/chunks", async (c) => {
  ...
  const id = crypto.randomUUID();
  const account_id = "acc_demo";
  await c.env.DB.prepare("INSERT INTO knowledge_chunks (id, account_id, source_id, content) VALUES (?, ?, ?, ?)").bind(id, account_id, sourceId, body.content).run();
...
// .wrangler/tmp/bundle-pGnyKW/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-pGnyKW/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.8307144196014873.js.map
