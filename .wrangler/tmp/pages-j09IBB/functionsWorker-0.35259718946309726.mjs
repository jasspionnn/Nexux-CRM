...
// ../.wrangler/tmp/bundle-ks89a1/strip-cf-connecting-ip-header.js
...
app.get("/seed-db", async (c) => {
  try {
    await c.env.DB.prepare(`
      ...
      VALUES ('acc_demo', 'Tech Solutions Ltda', 'Jo\xE3o Silva', 'joao@tech.com', 'active', 'pro', '2025-12-31T23:59:59Z')
...
app.post("/users", async (c) => {
  try {
    ...
    await c.env.DB.prepare("INSERT INTO users (id, account_id, name, email, password, role, status, team_id, avatar) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(id, account_id, body.name, body.email, "temp_password", body.role, body.status, team_id, avatar).run();
...
app.post("/leads/:id/notes", async (c) => {
  ...
  const body = await c.req.json();
  const id = crypto.randomUUID();
  await c.env.DB.prepare("INSERT INTO notes (id, lead_id, content, author_name) VALUES (?, ?, ?, ?)").bind(id, leadId, body.content, body.author_name || "Usu\xE1rio").run();
...
// ../.wrangler/tmp/pages-j09IBB/functionsRoutes-0.20078419772144263.mjs
var routes = [
  {
    routePath: "/api/:route*",
    mountPath: "/api",
...
// ../.wrangler/tmp/bundle-ks89a1/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-ks89a1/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.35259718946309726.mjs.map
