...
// ../.wrangler/tmp/bundle-n56Hab/strip-cf-connecting-ip-header.js
...
app.post("/custom-fields", async (c) => {
  ...
  const id = crypto.randomUUID();
  const account_id = "acc_demo";
  await c.env.DB.prepare("INSERT INTO custom_fields (id, account_id, name, type, context) VALUES (?, ?, ?, ?, ?)").bind(id, account_id, body.name, body.type, body.context).run();
...
app.post("/webhooks", async (c) => {
  ...
  const id = crypto.randomUUID();
  const account_id = "acc_demo";
  await c.env.DB.prepare("INSERT INTO webhooks (id, account_id, name, url, events, is_active) VALUES (?, ?, ?, ?, ?, ?)").bind(id, account_id, body.name, body.url, "all", body.active ? 1 : 0).run();
...
app.post("/users", async (c) => {
  ...
  const id = crypto.randomUUID();
  const account_id = "acc_demo";
  await c.env.DB.prepare("INSERT INTO users (id, account_id, name, email, password, role, status) VALUES (?, ?, ?, ?, ?, ?, ?)").bind(id, account_id, body.name, body.email, "temp_password", body.role, body.status).run();
...
// ../.wrangler/tmp/pages-pnFOoi/functionsRoutes-0.2793348477537527.mjs
var routes = [
  {
    routePath: "/api/:route*",
    mountPath: "/api",
...
// ../.wrangler/tmp/bundle-n56Hab/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-n56Hab/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.8716332448053856.mjs.map
