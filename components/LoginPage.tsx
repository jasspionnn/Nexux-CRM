  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
        if (isRegistering) {
            ...
            const result = registerAccount(name, email, password, companyName);
            if (result !== true) {
            ...
            if (result !== true) {
                setError(typeof result === 'string' ? result : 'Erro ao fazer login.');
                setLoading(false);
            }
        }
    }, 800);
        ...
        <div className="p-8 pb-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button 
                    type="submit"
                    disabled={loading}
                    className="w-full bg-blue-600 text-white font-bold py-3 rounded-xl hover:bg-blue-700 transition-all shadow-lg shadow-blue-200 active:scale-[0.98] flex justify-center items-center gap-2 mt-4"
                >
                    {loading ? (
